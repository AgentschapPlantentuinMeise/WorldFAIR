...
journal.datasets <- data.uniq %>%
  mutate(id=stringr::str_remove(id, "doi:")) %>%
  filter(grepl(paste(journal.suffix$suffix, collapse = '|'), id))
dim(journal.datasets)
journal.datasets %>%
  dplyr::count(source)
...
licenses[licenses$license == "https://doi.org/10.4121/resource:terms_of_use",]$license <- '4TU.Centre'
...
p <- licenses.count %>%
  ...
  ggplot2::annotate("text", x=grep("In Copyright", licenses.count$license), y=licenses.count$n[which(licenses.count=="In Copyright")]+100,
                    ...
                    color="grey", size=6,angle=0,fontface="bold", hjust=0)
p
ggsave('../results/licenses-types-wo-figshare.png', p, dpi = 300)
...
p <- subtypes.top %>%
  ...
  ggtitle("Most common file formats in datasets") +
  theme_ipsum(
    base_family = 'Arial'
  ) +
  coord_flip() +
...
p
ggsave('../results/top20-file-formats.png', p, dpi = 300)
...
p <- subtypes.top %>%
  ...
  ylab("Number of files")
p
ggsave('../results/file-formats-wo-figshare.png', p, dpi = 300)
...
plt <- interaction_kw_top20 %>%
  ...
  theme(plot.margin = margin(0.01, 0.05, 0.02, 0.01, "npc"))
plt
ggsave('../results/datasets-keywords.png', plt, dpi = 150)
